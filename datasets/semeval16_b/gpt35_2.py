...
db = VectorDatabase()
db.load_texts('FM.pkl')
db.load_faiss('FM.faiss')

tqdm.pandas(desc="Computing embeddings")
df = pd.read_csv("semeval16.csv", encoding='utf-8')

df = df[df['target'] == 'Feminist_Movement']
df = df.reset_index()
for i in tqdm(range(len(df))):
    text = df.loc[i, 'text']
    information = db.query(text, 200)
    background = get_reply(text, "Feminist Movement", information)
    df.loc[i, 'background'] = background

output_path = "./covid0.csv"
df.to_csv('Feminist_Movement.csv', index=False, encoding='utf-8')
