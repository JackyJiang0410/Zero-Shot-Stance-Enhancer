    ...
    while count < 3:
        try:
            completion = client.chat.completions.create(
                model="gpt-3.5-turbo",
                messages=[
                    {"role": "system", "content": "You're an expert in the field of stance testing and are good at "
                                                  "adding background knowledge based on sentences."},
...
db = VectorDatabase()
db.load_texts('HOME.pkl')
db.load_faiss('HOME.faiss')
...
df = df[df['Target 1'] == 'stay at home orders']
df = df.reset_index()
for i in tqdm(range(len(df))):
    text = df.loc[i, 'Tweet']
    information = db.query(text, 200)
    background = get_reply(text, "stay at home orders", information)
    df.loc[i, 'background'] = background

output_path = "./covid0.csv"
df.to_csv("home.csv", index=False, encoding='utf-8')
